    ...
    do
    {
        ...
        if(err == TX_SUCCESS)
        {
            ...
            switch(rtt_op_data->msg_id)
            {
                ...
                case RTT_OUTPUT_MESSAGE_APP_VAL:
                {
                    /* assign lease time to local buffer. */
                    ULONG lease_time = *(ULONG*)rtt_op_data->p_msg;
                ...
                case RTT_OUTPUT_MESSAGE_APP_PRINT_GATEWAY:
                {
                    union IP dns_gateway;
                    char ipv4_address[64] = {'\0'};
                    /* assign ipv4 address to local buffer. */
                    ULONG dns_gateway_addr = *(ULONG*)rtt_op_data->p_msg;
                    dns_gateway.ip_address = dns_gateway_addr;
                    sprintf(ipv4_address, "%d.%d.%d.%d", dns_gateway.IP_t.ip_a, dns_gateway.IP_t.ip_b, dns_gateway.IP_t.ip_c, dns_gateway.IP_t.ip_d);
                    APP_PRINT("\r\nSuccessfully pinged DNS gateway : %s\r\n", ipv4_address);
                }
                break;
