...
TX_THREAD usb_network_thread;
void usb_network_thread_create(void);
static void usb_network_thread_func(ULONG thread_input);
static uint8_t usb_network_thread_stack[4096] BSP_PLACE_IN_SECTION(".stack.usb_network_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
...
void usb_network_thread_create(void)
{
    ...
    UINT err;
    err = tx_thread_create (&usb_network_thread, (CHAR*) "USB Network Thread", usb_network_thread_func, (ULONG) NULL,
                            &usb_network_thread_stack, 4096, 20, 20, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
        tx_startup_err_callback (&usb_network_thread, 0);
    }
}
