...
TX_QUEUE g_reset_queue;
static uint8_t queue_memory_g_reset_queue[4];
void tx_startup_err_callback(void *p_instance, void *p_data);
TX_EVENT_FLAGS_GROUP g_usb_event_flags;
void tx_startup_err_callback(void *p_instance, void *p_data);
void g_common_init(void)
{
    UINT err_g_reset_queue;
    err_g_reset_queue = tx_queue_create (&g_reset_queue, (CHAR*) "Reset Queue", 1, &queue_memory_g_reset_queue,
                                         sizeof(queue_memory_g_reset_queue));
    if (TX_SUCCESS != err_g_reset_queue)
    {
        tx_startup_err_callback (&g_reset_queue, 0);
    }
    UINT err_g_usb_event_flags;
    err_g_usb_event_flags = tx_event_flags_create (&g_usb_event_flags, (CHAR*) "New Event Flags");
